...
import { getMyself, markNotificationsAsRead, setSelectedBrand, searchArticlesCatalog } from '../redux/adminSlice';
...
const Navbar = () => {
  ...
  // Sync nav search query if on /parts
  useEffect(() => {
  ...
  const handleNavSearchSubmit = (e) => {
    ...
    if (location.pathname === '/parts') {
      ...
      if (currentQ === q) {
        ...
        navigate(`/parts?q=${encodeURIComponent(q)}`);
      }
    } else {
      navigate(`/parts?q=${encodeURIComponent(q)}`);
    }
  ...
  return (
    <header className="h-16 bg-slate-900/95 backdrop-blur-md px-6 flex items-center justify-between sticky top-0 z-20 border-b border-slate-800 select-none">
      ...
      <form
        ...
        className="flex items-center gap-2 bg-slate-950/80 px-3 py-1.5 rounded-lg w-72 md:w-80 border border-slate-800 focus-within:border-slate-600 focus-within:bg-slate-950 focus-within:ring-2 focus-within:ring-slate-700/50 transition-all"
      >
        <button
          type="submit"
          className="text-slate-400 hover:text-white transition-colors cursor-pointer shrink-0"
          title="Hit TecDoc search"
        >
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-slate-800">
              {allNotifications.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center p-6 text-center">
                  <BellSlashIcon className="w-8 h-8 text-slate-600 mb-2" />
