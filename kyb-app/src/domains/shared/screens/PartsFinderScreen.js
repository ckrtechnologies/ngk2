...
const rawQuery = typeof overrideQuery === 'string' ? overrideQuery : partNumber;
    ...
    if (!trimmed) {
      Toast.show({
        ...
        text2: 'Please enter or tap a KYB part number below.',
      });
      return;
    }
    ...
    try {
      ...
      try {
        const restRes = await apiFunction(
          `${articlesByPartApi}?searchQuery=${encodeURIComponent(trimmed)}&brand=kyb`,
          [],
          {},
          'GET',
          false
      ...
      // Strict KYB brand isolation (exclude NGK/NTK)
      results = (results || []).filter((p) => {
        const b = (p.brandName || p.mfrName || p.brand || p.dataSupplierName || '').toUpperCase();
        const sup = Number(p.dataSupplierId || 0);
        return (b.includes('KYB') || sup === 7729) && !b.includes('NGK') && !b.includes('NTK');
      });
      ...
      >
        ...
        {searchMode === 'vehicle' ? (
          ...
          <View style={styles.partSearchContainer}>
            ...
            <AppInput
              placeholder="e.g. 333729, 341307, 133002"
              value={partNumber}
            ...
            <View style={styles.quickSearchRow}>
              ...
              {['333729', '341307', '133002', '343298', '553228'].map((q) => (
                <TouchableOpacity
                  key={q}
