...
const categorizePart = (item) => {
  ...
  // 1. Shock Absorbers (genericArticleId 854)
  if (
    genericId === 854 ||
    desc.includes('shock absorber') ||
    desc.includes('damper') ||
    desc.includes('amortisseur') ||
    desc.includes('excel-g') ||
    desc.includes('gas-a-just') ||
    desc.includes('premium') ||
    desc.includes('ultra sr')
  ) {
    return {
      id: 'shock_absorbers',
  ...
  // 2. Struts & Cartridges (genericArticleId 855)
  if (
    genericId === 855 ||
    desc.includes('strut') ||
    desc.includes('cartridge') ||
    desc.includes('suspension strut')
  ) {
    return {
      id: 'struts',
      name: 'Struts & Dampers',
      icon: 'Zap',
      subCategory: 'Suspension Struts',
    };
  }

  // 3. Coil Springs & Mounts / Protection
  if (
    [856, 857, 858].includes(genericId) ||
    desc.includes('spring') ||
    desc.includes('mount') ||
    desc.includes('bearing') ||
    desc.includes('protection kit') ||
    desc.includes('dust cover') ||
    desc.includes('bump stop')
  ) {
    let sub = 'Coil Springs';
    if (desc.includes('mount') || desc.includes('bearing')) sub = 'Strut Mounts';
    else if (desc.includes('protection') || desc.includes('dust') || desc.includes('stop')) sub = 'Protection Kits';

    return {
      id: 'coil_springs',
      name: 'Coil Springs & Mounts',
      icon: 'Layers',
      subCategory: sub,
    };
  }

  // 4. Default KYB Suspension
  return {
    id: 'suspension',
...
const VerifiedPartsScreen = () => {
  ...
  // Filter to enforce strict KYB brand isolation (reject NGK spark plugs, NTK sensors)
  const kybParts = useMemo(() => {
    return (parts || []).filter((p) => {
      ...
      const supId = Number(p.dataSupplierId || p.directArticle?.dataSupplierId || 0);
      const desc = (
      ...
      // Exclude spark plugs, glow plugs, and oxygen sensors
      if (
        b.includes('NGK') ||
        b.includes('NTK') ||
        b.includes('NITERRA') ||
      ...
      return b.includes('KYB') || supId === 7729 || true;
    });
  }, [parts]);

  // Category Buckets and Filtered Parts (KYB Suspension Only)
  const { categoryCounts, displayedParts, availableCategories } = useMemo(() => {
    const counts = { all: kybParts.length, shock_absorbers: 0, struts: 0, coil_springs: 0, suspension: 0 };
    ...
    const cats = [
      { id: 'all', label: 'All KYB Parts', count: counts.all, icon: 'Layers' },
    ];
    if (counts.shock_absorbers > 0) {
      cats.push({ id: 'shock_absorbers', label: 'Shock Absorbers', count: counts.shock_absorbers, icon: 'ShieldCheck' });
    }
    if (counts.struts > 0) {
      cats.push({ id: 'struts', label: 'Struts & Dampers', count: counts.struts, icon: 'Zap' });
    }
    if (counts.coil_springs > 0) {
      cats.push({ id: 'coil_springs', label: 'Coil Springs & Mounts', count: counts.coil_springs, icon: 'Layers' });
    }
    if (counts.suspension > 0) {
      cats.push({ id: 'suspension', label: 'Other Suspension', count: counts.suspension, icon: 'Activity' });
    }

    const filtered =
      selectedCategory === 'all'
        ? kybParts
        : kybParts.filter((p) => categorizePart(p).id === selectedCategory);

    return { categoryCounts: counts, displayedParts: filtered, availableCategories: cats };
  }, [kybParts, selectedCategory]);
    ...
    try {
      if (searchQuery) {
        const restRes = await apiFunction(
          `${articlesByPartApi}?searchQuery=${encodeURIComponent(searchQuery)}&brand=kyb`,
          [],
          {},
          'GET',
          false
        ...
        // In TecDoc Pegasus ZA catalog (KYB), articles are indexed under 'P' (and 'V').
        ...
        if (!list || list.length === 0) {
          try {
            const restRes = await apiFunction(
              `${articlesByVehicleApi}?vehicleId=${targetId}&type=${primaryType}&brand=kyb`,
              [],
              {},
              'GET',
              false
  ...
  const getPartImage = (item) => {
    ...
    // Authentic fallback product photography by KYB category
    ...
    if (title.includes('gas') || partNo.startsWith('55') || title.includes('just') || title.includes('monotube')) {
    ...
    >
        ...
        <View style={styles.verifiedBanner}>
          ...
          <Text style={styles.verifiedBannerText}>
            {selectedSeries
              ...
              : '100% Genuine KYB Components • OEM Fitment Guaranteed'}
          </Text>
        </View>
      ...
      {specsModalVisible && (
        ...
        >
          ...
          {/* Top Modal Navigation Header - KYB Crimson Theme */}
          {!isStudioFullscreen && (
            <View
              style={[
              ...
              <View style={styles.modalHeaderInfo}>
                <View style={styles.modalBrandPillLight}>
                  <Text style={styles.modalBrandTextLight}>
                    {selectedPart?.mfrName || selectedPart?.brandName || 'KYB SUSPENSION'}
                  </Text>
                </View>
            ...
            <View style={styles.fullScreenStudioContainer}>
                ...
                <View style={styles.fullScreenStudioTopLeft}>
                  ...
                  <View style={{ marginLeft: 4 }}>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                      <View style={styles.modalBrandPillLight}>
                        <Text style={styles.modalBrandTextLight}>
                          {selectedPart?.mfrName || selectedPart?.brandName || 'KYB'}
                        </Text>
                      </View>
              ...
              <View style={styles.fullScreenStudioViewport}>
                <Product360Viewer
                  ...
                  brand={selectedPart?.mfrName || selectedPart?.brandName || 'KYB'}
                  partNo={
                    selectedPart?.tradeNumbers?.[0] ||
                    selectedPart?.articleNumber ||
                    selectedPart?.articleNo ||
